...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let dropdownMenuDove = locationSection.querySelector('.location-menu');
...
//-------------GESTIONE MENU BARRA RICERCA----------------------------

// Funzione per gestire l'apertura/chiusura dei menu
function handleMenuToggle(targetId) {
  console.log(`Gestione del menu con ID: ${targetId}`);
  const menus = document.querySelectorAll('.dropdown-menu, .calendar-menu');
  menus.forEach(menu => {
      if (menu.id !== targetId) {
        console.log(`Chiudo il menu: ${menu.id}`);
          menu.classList.remove('show'); // Chiude gli altri menu
      }
  });
  const targetMenu = document.getElementById(targetId);
  if (targetMenu) {
    console.log(`Alterno lo stato del menu: ${targetMenu.id}`);
      targetMenu.classList.toggle('show'); // Alterna apertura/chiusura
  } else {
    console.error(`Menu con ID "${targetId}" non trovato nel DOM!`);
}
}

// Listener globale per chiudere i menu al clic fuori
document.addEventListener('click', (event) => {
  const isMenu = event.target.closest('.dropdown-menu, .calendar-menu') !== null;
  const isButton = event.target.closest('.hover-section') !== null;
  if (!isMenu && !isButton) {
      document.querySelectorAll('.dropdown-menu, .calendar-menu').forEach(menu => {
          menu.classList.remove('show');
      });
  }
});

// Aggiungi i listener ai pulsanti delle sezioni
document.getElementById('location-section').addEventListener('click', (event) => {
  event.stopPropagation();
  handleMenuToggle('location-menu');
});

document.getElementById('check-in-section').addEventListener('click', (event) => {
  event.stopPropagation();
  handleMenuToggle('checkin-menu');
});
